...
import numpy as np
import unittest
from automate_image_alignment import align_and_register_images, save_registered_images, main
...
class TestAutomateImageAlignment(unittest.TestCase):
    ...
    def test_main(self):
        main(self.image_dir, self.output_dir, self.fformat, self.pad, self.deg, self.avg_over, self.subpx, self.color)
        self.assertTrue(os.path.exists(self.output_dir))
        for i in range(10):
            self.assertTrue(os.path.exists(os.path.join(self.output_dir, f'registered_image_{i}.{self.fformat}')))
